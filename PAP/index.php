<?php
session_start();
// Calculate base path for AJAX requests so that the app works from a
// subdirectory on shared hosting as well as from the web root.
...
<script>
    // Base path used by fetch() requests
...
function loadReviews(showAll = showingAllReviews) {
  // Add timestamp query parameter to avoid caching issues
  fetch(`${BASE_PATH}/reviews.php?t=${Date.now()}`)
    .then(r=>r.json())
    .then(data=>{
      allReviewsData = data;
      ...
      reviewForm.onsubmit = function(e) {
        ...
        fetch(`${BASE_PATH}/user_status.php`, { credentials: 'same-origin' })
          ...
          .then(user => {
            ...
            fetch(`${BASE_PATH}/adicionar_review.php`, {
              method: 'POST',
            ...
            .then(data => {
              if (data.success) {
                ...
                // Reload reviews using the same visibility state
                loadReviews(showingAllReviews);
              } else {
                msg.style.color = '#e04040';
